...
export const subsetToBinary = async (
  ...
  // Casts: TS 5.7's typed-array generic parameter (`Uint8Array<ArrayBufferLike>`)
  // is narrower than what the third-party woff2/hbsubset shim signatures
  // declare. The runtime values are plain ArrayBuffers from compress/subset;
  // the cast is forward-compat, not a runtime change.
  const decompressedBinary = decompress(arrayBuffer).buffer as ArrayBuffer;
