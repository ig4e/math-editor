...
interface Blob {
  // `FileSystemHandle` is a DOM global (lib.dom.d.ts); upstream imported
  // it from `browser-fs-acces` (note: typo in upstream — missing 's')
  // back when that package re-exported the DOM type. We use the global
  // directly to dodge both the typo and the resolution miss.
...
// Vite resolves font + asset imports to URLs at build time. TS only needs
// to know the import returns a string-shaped module.
