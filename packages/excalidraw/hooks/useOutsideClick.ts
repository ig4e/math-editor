...
export function useOutsideClick<T extends HTMLElement>(
  ...
  useEffect(() => {
    function onOutsideClick(event: Event) {
      ...
      // `T extends HTMLElement` — _event.target is assignable to
      // HTMLElement at runtime; TS 5 needs the cast made explicit.
      const isInsideOverride = isInside?.(
        _event as unknown as Event & { target: HTMLElement },
        ref.current,
      );
